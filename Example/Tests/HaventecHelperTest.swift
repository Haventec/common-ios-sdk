...
class HaventecHelperTest: XCTestCase {
    ...
    func testGenerateSalt() {
        if let salt: String = try? HaventecHelper.generateSalt(size: 128) {
    ...
    func testHashPin() {
        let salt: String = try! HaventecHelper.generateSalt(size: 128)
        
        let hashedPin: String = HaventecHelper.hashPin(salt: salt, pin: "1234")
    ...
    func testSameSaltWithDifferentPin() {
        ...
        let hashedPinA: String = HaventecHelper.hashPin(salt: salt, pin: "1234")
        let hashedPinB: String = HaventecHelper.hashPin(salt: salt, pin: "7890")
