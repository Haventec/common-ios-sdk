...
class HaventecCommonTest: XCTestCase {
    ...
    func testGenerateSalt() {
        if let salt: String = try? HaventecCommon.generateSalt(size: 128) {
    ...
    func testHashPin() {
        let salt: String = try! HaventecCommon.generateSalt(size: 128)
        
        let hashedPin: String = HaventecCommon.hashPin(salt: salt, pin: "1234")
    ...
    func testSameSaltWithDifferentPin() {
        ...
        let hashedPinA: String = HaventecCommon.hashPin(salt: salt, pin: "1234")
        let hashedPinB: String = HaventecCommon.hashPin(salt: salt, pin: "7890")
